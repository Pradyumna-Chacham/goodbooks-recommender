...
st.title("🧮 SVD (User-based)")

ui = get_ui_artifacts()
A = get_mm_artifacts()

if "svd_user_id" not in st.session_state:
    st.session_state.svd_user_id = sample_random_user(A)
...
with c1:
    user_id = st.number_input(
        "User ID", min_value=1, value=int(st.session_state.svd_user_id), step=1
    )
with c2:
    if st.button("Apply persona", use_container_width=True):
        chosen = PERSONAS.get(persona)
        st.session_state.svd_user_id = sample_random_user(A) if chosen is None else int(chosen)
        st.rerun()
with c3:
...
st.session_state.svd_user_id = int(user_id)

k = st.slider("How many recommendations?", 5, 50, 10)

book_ids = recommend_for_user("SVD", user_id=int(user_id), k=int(k), A=A)
render_book_recommendations(book_ids, ui, page_key="svd_user", show_interaction_count=True)
